...
def move_children_behind_current_commit(
    scale, time_direction, min_separation, commit, commits, placement
):
    (_, _, text) = commit
    width, height = get_box_dims(scale, text)
    ...
    for child in children:
        ...
        if time_direction in [0, 90]:
            ...
            if (angle - time_direction) < 0:
                ...
                shift = None
        if shift == "min":
            placement[k][_sep_dim] = min(
                placement[k][_sep_dim], placement[commit][_sep_dim] - min_separation
            )
        elif shift == "max":
            placement[k][_sep_dim] = max(
                placement[k][_sep_dim], placement[commit][_sep_dim] + min_separation
            )
        else:
        ...
        if time_direction in [90, 180]:
            placement[k][_time_dim] = max(
                placement[k][_time_dim], placement[commit][_time_dim] + min_separation
            )
        else:
            placement[k][_time_dim] = min(
                placement[k][_time_dim], placement[commit][_time_dim] - min_separation
            )

        placement = move_children_behind_current_commit(
            scale, time_direction, min_separation, k, commits, placement
        )
    return placement


def point_inside_box(point_xy, box_center_xy, box_width, box_height):
...
def arrow_head(arrow_origin_xy, box_center_xy, box_width, box_height):
    """
    ...
    for f in range(num_steps + 1):
        ...
        if not point_inside_box((x, y), box_center_xy, box_width, box_height):
            return x, y
